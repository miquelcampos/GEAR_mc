...
class Guide(ComponentGuide):
    ...
    def addParameters(self):
        ...
        self.pFkRef       = self.addParam("fkref", c.siInt4, None, 0, None)
        self.pFkRefArray  = self.addParam("fkrefarray", c.siString, "")
    ...
    def addLayout(self):
        ...
        fkItemsCode = "fkrefItems = []" +"\r\n"+\
                      "if PPG."+self.pFkRefArray.scriptName+".Value:" +"\r\n"+\
                      "    a = PPG."+self.pFkRefArray.scriptName+".Value.split(',')" +"\r\n"+\
                      "    for i, v in enumerate(a):" +"\r\n"+\
                      "        upvrefItems.append(a[i])" +"\r\n"+\
                      "        upvrefItems.append(i)" +"\r\n"+\
                      "item.UIItems = fkrefItems" +"\r\n"
...
#        item.setAttribute(c.siUINoLabel, True)
        ...
        # FK References
        group = tab.addGroup("FK References")

        row = group.addRow()
        item = row.addEnumControl(self.pFkRef.scriptName, [], "FK", c.siControlCombo)
        item.setCodeAfter(fkItemsCode)
        row.addButton("PickFkRef", "Pick New")
        row.addButton("DeleteFkRef", "Delete")
#        item = group.addItem(self.pFkRefArray.scriptName, "")
#        item.setAttribute(c.siUINoLabel, True)
