...
def reloadModule(name="gear", silent=True):
    ...
    errors = []
    errors = __reloadRecursive(path, name, errors, silent)

    setDebug(debugMode)

    return errors

def __reloadRecursive(path, parentName, errors=[], silent=True):
    ...
    for root, dirs, files in os.walk(path, True, None):
        ...
        for dirName in dirs:
            errors = __reloadRecursive(path+"/"+dirName, parentName+"."+dirName, errors, silent)
        break
