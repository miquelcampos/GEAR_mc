...
class Component(MainComponent):
    ...
    def addObjects(self):
        ...
        if self.settings["roll"] == 0: # Roll with a controler
            self.roll_ctl = self.addCtl(self.ik_ref, "roll_ctl", self.ik_ref.Kinematics.Global.Transform, self.color_ik, "bendedarrow2", w=self.length2*.5*self.n_factor, po=XSIMath.CreateVector3(0,self.length2*.5*self.n_factor,0))
            if self.negate:
    ...
    def addOperators(self):
        ...
        if self.settings["roll"] == 0:
            ...
            self.ref_chn.eff.Kinematics.AddConstraint("Pose", self.ik_ref, True)
            self.end_chn.eff.Kinematics.AddConstraint("Pose", self.ik_ref, True)
...
#                                                        +"("+str(maxLength - restLength)+")))")
        ...
        # References Nulls ---------------------------------
        fk_ref = [self.fk0_ctl, self.fk1_ctl, self.fk2_ctl, self.fk3_ctl, self.fk_ref]
        ...
        # Twist references ---------------------------------
        par.addExpression(self.pDriver, "ctr_dist(%s, %s) + ctr_dist(%s, %s) + ctr_dist(%s, %s)"
